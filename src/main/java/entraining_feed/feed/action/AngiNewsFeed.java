package entraining_feed.feed.action;

import entraining_feed.feed.config.YamlConfig;
import entraining_feed.feed.data.AngiStaticData;
import lombok.Data;
...
public class AngiNewsFeed extends Feed {
    ...
    public void action() throws IOException {
        ...
        log.info("url: " + url);
        log.info("keywords: " + AngiStaticData.keywordsAndGroups.keySet());
    ...
    private void addNovostToCategorisedMap(String novostURL, String date, String title, String body) {
        for (int i = 0; i < AngiStaticData.orderedKeywords.size(); i++) {
            String keyword = AngiStaticData.orderedKeywords.get(i);
            if (!keyword.contains(" ")) {
                if (title.contains(keyword) || body.contains(keyword)) {
                    addNewToSpecificList(novostURL, title, body, date, AngiStaticData.keywordsAndGroups.get(keyword));
                    return;
                }
            } else {
    ...
    private void addNewToSpecificList(String novostURL, String title, String body, String date, String keyword) {
        List<Novost> novostList = groupedNews.get(keyword);
        if (novostList == null) {
            novostList = new ArrayList<>();
        }
        log.info(String.format("keyword %s found, novost: %s", keyword, title));
        createAndAddNewToList(novostURL, title, body, date, novostList);

        groupedNews.put(keyword, novostList);
    }
    ...
    private void extractNewsURLsToList(String workingURL) throws IOException {
        ...
        while (true) {
            ...
            Document document = Jsoup.connect(workingURL).timeout(300000).get();
    ...
    public void toExcel() throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (Map.Entry<String, List<Novost>> entry : groupedNews.entrySet()) {
                performToExcel(workbook, entry.getKey(), entry.getValue(), columns);
            }
            ...
            log.info(String.format("exported to excel %d news", AngiStaticData.newsCounter));
        }
    }
}
